...
public class Player : GameCharacter {
    ...
    private void Update()
    {
        ...
        if (CurrentHungerValue <= 0) {
            ...
            BM.addBuff(BuffStock.findBuff("Hunger"));
    ...
    public override void saveData() {
        ...
        foreach(Item i in ItemList) {
            data.addItemIndex(i.Name, Bag.findItemGrid(i).ItemCount, i.getStatus());
        }
        // 更新Buff列表
        foreach(Buff buff in BuffList) {
            data.BuffReference.Add(new BuffLoadIndex(buff.Name,buff.Duration,buff.getStatus()));
        }
        // 更新DataAssets
    ...
    public override void loadData() {
        ...
        foreach(ItemLoadIndex i in playerdata.ItemReference) {
            Item item = ItemStock.Instance.getItemByName(i.Name);
        ...
        if (playerdata.BuffReference != null) {
            foreach (BuffLoadIndex b in playerdata.BuffReference) {
                Buff buff = BuffStock.findBuff(b.Name);
