...
public class LadyD : NPC {
    ...
    // 攻击
    public override bool hit(GameCharacter character, DamageEnum damageType, float damage) {
        return base.hit(character, damageType, damage);
    ...
    // 限定类型、伤害数值
    public override void attack(GameCharacter target, DamageEnum damageType, float damage) {
        hit(target, damageType, damage);
    }
    // 随机攻击（普通攻击）
    public override void attack(GameCharacter target) {
        if(CurrentCoolDown == CoolDown) {
            if (checkHit(target)) {
                hit(target, getDamageType(), BasicDamage);
            }
            stop();
            Locked = true;
            Invoke("cure", 1.0f);
    ...
    // 获得伤害类型
    public override DamageEnum getDamageType() {
        // 该角色只会造成一种类型的伤害
        return DamageEnum.HEAVY_ATTACK;
    }
    // 添加攻击效果
    // 限定效果
