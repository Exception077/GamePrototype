...
public class EquipmentGrid : ItemGrid {
    ...
    void Update() {
        if (DS.CurrentItemGrid != null && DS.CurrentItemGrid != this) {
            if (DS.CurrentItemGrid.MyItem.EquipType == EquipType)
            HighlightComponent.SetActive(true);
        } else {
            HighlightComponent.SetActive(false);
        }
        if (MouseIn && Input.GetMouseButtonUp(0) && DS.CurrentItemGrid != null && DS.CurrentItemGrid != null && DS.CurrentItemGrid.MyItem.EquipType == EquipType) {
            print("Equip!");
